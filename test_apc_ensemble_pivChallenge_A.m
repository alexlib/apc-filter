...
image_dir = fullfile(get_image_repo, 'piv_challenge_2014_A', 'raw');
image_base_name = 'A_';
num_digits = 5;
image_ext = '.tif';
start_image = 1;
...
for n = 1 : num_images_correlate
    ...
    for k = 1 : num_regions
        ...
        [tx_apc(k, n), ty_apc(k, n)] = subpixel(apc_plane,...
            region_width, region_height, subpix_weights, ...
            3, 0, [dp_equiv_apc_x, dp_equiv_apc_y]); 
        
        scc_ens = scc_ens + scc_plane;
        rpc_ens = rpc_ens + rpc_plane;
        
        spectral_corr_ens = spectral_corr_ens + cc_spect;
        
    end
    
    apc_plane = abs(fftshift(ifft2(fftshift(phaseOnlyFilter(spectral_corr_ens) .* apc_filter))));
