...
% Number of NEQ ensembles
num_ens_neq = num_regions;
...
cc_neq_abs_sq_full = zeros(region_height, region_width);
...
ac_avg = zeros(region_height, region_width) + 1i * zeros(region_height, region_width);
...
for k = 1 : num_ens_neq
    ...
    % Fourier transforms of uncorrelated images
    f1 = fftn(region_01 .* g_win, [region_height, region_width]);
    f2 = fftn(region_02 .* g_win, [region_height, region_width]);
    
    % Auto correlations
    ac_01 = fftshift(f1 .* conj(f1));
    ac_02 = fftshift(f2 .* conj(f2));
   
    ac_avg = ac_avg + (ac_01 + ac_02)/2;
    
    % Complex cross correlation of the uncorrelated regions
    cc_neq_cur = fftshift(f1 .* conj(f2));
    
    % Running full non-coresponding correlation
    cc_neq_full = cc_neq_full + cc_neq_cur;
   
    % Running magnitude of the current corresponding NCC
    cc_neq_abs_full = cc_neq_abs_full + abs(cc_neq_cur);
    
    % Running magnitude squared
    cc_neq_abs_sq_full = cc_neq_abs_sq_full + (abs(cc_neq_cur)).^2;
  
    % Running DC component of corresponding NCC
...
particle_filter = real(ac_avg - cc_neq_full);

% Scaled non-corresponding correlation
cc_neq_scaled = cc_neq_abs_sq_full / num_ens_neq;
...
cc_eq_abs_sq_full = zeros(region_height, region_width);
...
for k = 1 : num_regions
    ...
    % Subtracted
%     cc_eq_sub = abs(cc_eq_cur).^2 - (np_eff * (np_eff - 1))^2 * cc_neq_scaled ./ max(real(cc_neq_scaled(:)))^2;
        
      cc_eq_sub = (abs(cc_eq_cur)).^2 - cc_neq_scaled;
...
%     cc_eq_sub = cc_eq_cur - np_eff * (np_eff - 1) * cc_neq_scaled ./ max(real(cc_neq_scaled(:)));
    ...
    cc_eq_abs_sq_full = cc_eq_abs_sq_full + (abs(cc_eq_cur)).^2;
...
% n = (max(cc_eq_full(:)) / max(cc_neq_full(:)));

a = cc_eq_full ./ particle_filter;
b = cc_neq_full ./ particle_filter;
...
surf(abs(c), 'linewidth', 1); 
axis square;
set(gca, 'view', [0, 0]);


% g = fftshift(abs(ifft2(fftshift(c))));
% surf(g)

% n = (max(cc_eq_abs_full(:)) / max(cc_neq_abs_full(:)));
% 
% a = (cc_eq_abs_full ./ particle_filter);
% b = (cc_neq_abs_full ./ particle_filter);
...
% c_ift = fftshift(real((ifft2(fftshift(c)))));


% surf(c_ift, 'linewidth', 1);
